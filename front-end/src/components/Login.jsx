...
const Login = ({onSubmit, onSwitchMode}) => {
  ...
  const handleSubmit = async (e)=>{
        e.preventDefault()
        if(!rememberMe){
            toast.error('You must eneble "Remember Me" to login.')
            return
        }
        setLoading(true)
  ...
  return (
    <div className=" max-w-md bg-white w-full shadow-lg border border-purple-100 rounded-2xl p-8">
      ...
      <form onSubmit={handleSubmit} className=" space-y-4">
        ...
        <button type="submit" className={BUTTON_CLASSES} disabled={loading}>
          ...
          <p className=" text-center text-sm text-gray-600 mt-6">
            ...
            <button type="button" className=" text-purple-600 hover:text-purple-700 hover:underline font-medium transition-colors" onClick={handleSwitchMode}>Sign Up</button>
          </p>
